...
bot.on('message', function(message) {
	...
	if(message.content === 'd?help') {
		...
		var embed = new Discord.RichEmbed()
		.setColor("#226666")
		.addField("Voici votre profil: ", "Pseudo: **" + message.author.username + "**\nIdentifiant: **" + message.author.id + "**\nStatut: **"  + message.author.presence.status + "**\nCe compte a été crée le **" + date_moment.format('DD MMM YYYY') + if(message.author.presence.game.name == null) { "**Vous ne jouez a aucune activité**" } else { "**" + message.author.presence.game.name + "**" })
